...
import type { BuildPlanRecord } from "@founderos/core";
...
export function BuildExecution({ projectId, projectName, initialPlans }: { projectId: string; projectName: string; initialPlans: BuildPlanRecord[] }) {
  ...
  async function createPlan() {
    ...
    try {
      const plan = await call(`/api/projects/${projectId}/build`, {
        method: "POST",
        headers: { "content-type": "application/json" },
        body: JSON.stringify({ repository, objective: objective || undefined })
      });
  ...
  async function approve(planId: string) {
    ...
    try {
      replace(await call(`/api/projects/${projectId}/build/${planId}/approve`, { method: "POST" }));
      setMessage("Plan approved. GitHub publication is now unlocked.");
  ...
  async function publish(planId: string) {
    setBusy(planId); setMessage("");
    try {
      replace(await call(`/api/projects/${projectId}/build/${planId}/publish`, { method: "POST" }));
      setMessage("Branch and implementation issues published to GitHub.");
  ...
  async function copyWorkOrder(planId: string) {
    ...
    try {
      ...
      setMessage("Work order copied to clipboard.");
  ...
  return <div className="container" style={{ paddingTop: 28 }}>
    <div className="card">
      <div className="small muted">PHASE 2 · BUILD EXECUTION</div>
      <h2>{projectName}</h2>
      <p>Convert an evidence-backed project into an approval-gated GitHub delivery plan. FounderOS creates a dedicated branch and implementation issues; it never writes directly to the base branch.</p>
      <div className="grid2">
    ...
    {plans.map((plan) => <div className="card" key={plan.id} style={{ marginBottom: 14 }}>
      ...
      <div className="timeline" style={{ marginTop: 14 }}>
        {[...plan.tasks].sort((a, b) => a.order - b.order).map((task) => <div className="event" key={task.id}>
          ...
          {task.githubIssue ? <p><a href={task.githubIssue.url} target="_blank" rel="noreferrer">GitHub issue #{task.githubIssue.number}</a></p> : null}
        </div>)}
      </div>
