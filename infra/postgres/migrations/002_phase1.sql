...
alter table projects add column if not exists per_run_budget_usd numeric(12,6) not null default .50;
...
create table if not exists audit_events (id bigserial primary key,organization_id uuid not null references organizations(id) on delete cascade,project_id uuid references projects(id) on delete cascade,actor_user_id uuid,event_name text not null,properties jsonb not null default '{}'::jsonb,occurred_at timestamptz not null default now());
create table if not exists source_snapshots (id uuid primary key default gen_random_uuid(),project_id uuid not null references projects(id) on delete cascade,source_url text not null,content_hash text not null,content_type text,captured_text text not null,captured_at timestamptz not null default now(),unique(project_id,content_hash));
create table if not exists claims (id uuid primary key,project_id uuid not null references projects(id) on delete cascade,statement text not null,status text not null default 'hypothesis',confidence numeric(5,4) not null default .5 check(confidence>=0 and confidence<=1),created_at timestamptz not null default now());
create table if not exists claim_evidence (claim_id uuid not null references claims(id) on delete cascade,evidence_id uuid not null references evidence(id) on delete cascade,relationship text not null default 'supports',primary key(claim_id,evidence_id));
